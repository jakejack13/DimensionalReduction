...
while (a.iterations < a.maxIterations) || a.iterations > 10
  ...
  if conv < 0 || sum(isnan(a.W(:))) || sum(isinf(a.W(:))) || ...
    ...
    ddW = zeros(size(a.W));
    a.etaW = .8 * a.etaW;
    fprintf('change = %04g\tlowered etaW to %g\n',conv,a.etaW);
    conv = Inf;
    reject_w = 1;
  else 
    a.etaW = 1.01 * a.etaW;
    ...
    disp(['Iter ', num2str(a.iterations), ' updW, objective function : ' num2str(lastobjective/nPoints) ', conv: ' num2str(conv) ' change in W: ' num2str(paramchange) '\n']);
  ...
  if conv < 0 || sum(isnan(a.H(:))) || sum(isinf(a.H(:))) || ...
    ...
    ddH = zeros(size(a.H));
    a.etaH = .8 * a.etaH;
    fprintf('change = %04g\tlowered etaH to %g\n',conv,a.etaH);
    conv = Inf;
    reject_h = 1;
    ...
    lastobjective = objective;
    disp(['Iter ', num2str(a.iterations), ' updH, objective function : ' num2str(lastobjective/nPoints) ', conv: ' num2str(conv) ' change in H= ' num2str(paramchange) '\n']);
  end
  ...
  if ~reject_w & ~reject_h
    ...
    if (conv < a.eps) && a.iterations > 10 && conv_count == 1
      fprintf('final change in objective %g\n',conv);
      converged = 1;
      break;
...
fprintf('Traing time %g hours\n',(a.algorithm.training_time+cputime-starttime)/3600);

if converged
  fprintf('Converged after %d iterations, final objective: %g\n',a.iterations,objective/nPoints);
else  
  fprintf('Maximum of iterations (%d) reached\n',a.iterations);
end
